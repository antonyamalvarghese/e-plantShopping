    ...
    <div>
      ...
      {!showCart ? (
        <div className="product-grid">
          {showPlants ? (
            plantsArray.map((categoryBlock) => (
              <div key={categoryBlock.category} className="category-block">
                <h2 className="category-title">{categoryBlock.category}</h2>

                <div className="plants-grid">
                  {categoryBlock.plants.map((plant) => (
                    <div
                      key={`${categoryBlock.category}-${plant.name}`}
            ...
            <p style={{ color: "#fff", padding: "20px" }}>
              Click <b>Plants</b> to view available plants 🌿
            </p>
          )}
        </div>
      ) : (
